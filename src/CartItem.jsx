...
const CartItem = ({ onContinueShopping, onCheckoutShopping }) => {
    ...
    const calculateTotalAmount = () => {
        return cart.reduce((previous, current) => {
            return previous + parseFloat(current.cost.substring(1)) * current.quantity;
        }, 0);
    };
    ...
    const handleCheckoutShopping = (e) => {
        onCheckoutShopping && (onCheckoutShopping(e));
    }
    ...
    const handleDecrement = (item) => {
        dispatch(updateQuantity({ name: item.name, quantity: item.quantity > 0 ? item.quantity - 1 : 0 }));
    };
